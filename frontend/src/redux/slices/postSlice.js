...
export const setPosts = createAsyncThunk(
	...
	async (props, thunkAPI) => {
		...
		const data = await customFetch(fetchPosts);
...
export const addPost = createAsyncThunk('post/add', async (props, thunkAPI) => {
	...
	dispatch(showModal({}));
	const data = await customFetch(createPost, formData);
	if (!data) return rejectWithValue();
...
export const updatePost = createAsyncThunk(
	...
	async (props, thunkAPI) => {
		...
		dispatch(showModal({}));
		const data = await customFetch(updatePost, { id, form: formData });
		if (!data) return rejectWithValue();
...
export const likePost = createAsyncThunk(
	...
	async (props, thunkAPI) => {
		...
		const { dispatch, rejectWithValue, getState } = thunkAPI;
		const data = await customFetch(likePost, { id, add: !isLiked });
		if (!data) return rejectWithValue();
		if (getState().post.singlePost._id === id)
			dispatch(postSlice.actions.setSinglePost(data.post));
		dispatch(postSlice.actions.updatePosts(data.post));
...
export const commentPost = createAsyncThunk(
	...
	async (props, thunkAPI) => {
		...
		const { dispatch, rejectWithValue, getState } = thunkAPI;
		const data = await customFetch(commentPost, { id, comment });
		if (!data) return rejectWithValue();
		if (getState().post.singlePost._id === id)
			dispatch(postSlice.actions.setSinglePost(data.post));
		dispatch(postSlice.actions.updatePosts(data.post));
...
export const deletePost = createAsyncThunk(
	...
	async (props, thunkAPI) => {
		...
		dispatch(showModal({}));
		await customFetch(deletePost, { id });
